...
#include "base/Util.h"
#include "Plg/RangeHasMacroAstVst.h"
#include "Plg/CollectIncMacro_PPCb.h"
...
void PlgVst::letLRBraceWrapRangeAftBf(SourceLocation B, SourceLocation E, const char* whoInserted ){
 ...
 * @return
 */
void PlgVst::letLRBraceWrapStmtBfAfTk(Stmt *stmt, const char* whoInserted){
...
bool PlgVst::TraverseIfStmt(IfStmt *ifStmt){
  //region 若NULL，直接返回
  if(!ifStmt){
    return false;
  }
...
//    Util::printStmt(CI,"查看","暂时不对间接文件插入时钟语句",stmt, true); //开发用打印
  ...
  if(thenStmt && !Util::isAloneContainerStmt(thenStmt) )  {
    letLRBraceWrapStmtBfAfTk(thenStmt, "PlgThen");
  }

  Stmt *elseStmt = ifStmt->getElse();
  if(elseStmt && !Util::isAloneContainerStmt(elseStmt) ) {
    letLRBraceWrapStmtBfAfTk(elseStmt, "PlgElse");
  }
//endregion 自定义处理 完毕
...
bool PlgVst::TraverseWhileStmt(WhileStmt *whileStmt){
  //region 若NULL，直接返回
  if(!whileStmt){
    return false;
  }
...
//    Util::printStmt(CI,"查看","暂时不对间接文件插入时钟语句",stmt, true); //开发用打印
  ...
  if(bodyStmt && !Util::isAloneContainerStmt(bodyStmt) )  {
    letLRBraceWrapStmtBfAfTk(bodyStmt, "PlgWhl");
  }
...
//forEach和for很相似
bool PlgVst::TraverseForStmt(ForStmt *forStmt) {
  //region 若NULL，直接返回
  if(!forStmt){
    return false;
  }
...
//    Util::printStmt(CI,"查看","暂时不对间接文件插入时钟语句",stmt, true); //开发用打印
  ...
  if(bodyStmt && !Util::isAloneContainerStmt(bodyStmt) )  {
    letLRBraceWrapStmtBfAfTk(bodyStmt, "PlgFor");
  }
  //endregion
...
//forEach和for很相似
bool PlgVst::TraverseCXXForRangeStmt(CXXForRangeStmt *forRangeStmt) {
  //region 若NULL，直接返回
  if(!forRangeStmt){
    return false;
  }
...
//    Util::printStmt(CI,"查看","暂时不对间接文件插入时钟语句",stmt, true); //开发用打印
  ...
  if(bodyStmt && !Util::isAloneContainerStmt(bodyStmt) )  {
    letLRBraceWrapStmtBfAfTk(bodyStmt, "PlgForRange");
  }
  //endregion
...
bool PlgVst::TraverseSwitchStmt(SwitchStmt *switchStmt){
...
//      Util::printStmt(*Ctx,CI,"scK",msg,scK,true);//开发用
      ...
      letLRBraceWrapRangeAftBf(beginLoc, endLoc, "PlgSw");
