...
static llvm::cl::OptionCategory PlgAloneCategory("PlgAlone选项");

class _PlgAstAct : public ASTFrontendAction {
public:
    ...
    CreateASTConsumer(CompilerInstance &CI,
      ...
      return std::make_unique<PlgAstCnsm>(CI,mRewriter_ptr, &astContext, SM, langOptions);
    }
private:
...
int main(int Argc, const char **Argv) {
  const std::unique_ptr<tooling::FrontendActionFactory> &frontendActionFactory = clang::tooling::newFrontendActionFactory<_PlgAstAct>();
  int Result =   act_main(Argc,Argv,PlgAloneCategory,frontendActionFactory,"加花括号插件", false);
  return Result;
}
